...
def build_panel():
    ...
    for r in summary:
        with open(os.path.join(DOCS_DIR, "data.csv"), "a", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=fields); w.writerow(r)
    html = ["<!doctype html><html><head><meta charset='utf-8'><title>Pokémon Panel</title>"]
    html.append("<style>body{font-family:Arial,sans-serif;padding:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ddd;padding:8px;}th{background:#f5f5f5}</style></head><body>")
    html.append("<h1>Pokémon Price Panel</h1>")
    html.append("<table><thead><tr><th>Carta</th><th>Ahora</th><th>Market</th><th>Δ24h</th><th>Δ7d</th><th>Breakout</th></tr></thead><tbody>")
    for r in summary:
